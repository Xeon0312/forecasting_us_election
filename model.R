...
MODEL_PATH = "outputs/model/brms_model"

model <- brms::brm(vote_2020 ~ age_group + gender + education + state,
             data = poll_df,
             family = bernoulli(),
             file = MODEL_PATH 
             )
...
# Reading model
model <- read_rds(paste(MODEL_PATH, ".rds", sep = ""))
...
post_stratified_estimates <- 
  model %>%
  tidybayes::add_predicted_draws(newdata = poll_df) %>% 
